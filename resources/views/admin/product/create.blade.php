...
<div class="row my-3">
  <p class="display-4 text-center fw-bold">Tambah Produk</p>
  <p class="text-center">Kategori " {{session('category')->category_name ?? $category->category_name ?? 'Tidak diketahui' }} "</p>
</div>
    ...
    <div class="col-md-8">
      <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input id="type" type="hidden" class="form-control @error('type') is-invalid @enderror" name="type" value="{{ $category_type ?? session('category')->category_type ?? $category->category_type ?? 'Tidak diketahui' }}" placeholder="" required autocomplete="type" autofocus readonly="readonly">
          @error('type')
        ...
        <div class="form-group">
          ...
          <input id="category_id" type="hidden" class="form-control @error('category_id') is-invalid @enderror" name="category_id" value="{{ $category->id ?? session('category_id') ?? 'Tidak diketahui' }}" placeholder="" required autocomplete="category_id" autofocus readonly="readonly">
          @error('category_id')
              <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
              </span>
          ...
          <div id="stock-wrapper">
            <div class="stock-row d-flex align-items-center mb-2">
              <select name="stocks[0][id]" class="form-control me-2" style="width: 50%;" required>
                ...
                @php
                    // Cek dulu apakah variabel $stocks dari controller sudah ada
                    if (session('stocks')!=null) {
                        // Kalau belum ada, ambil dari session
