...
<a class="nav-link count-indicator" id="notificationDropdown" href="#" data-bs-toggle="dropdown">
  @if ($totalNotif > 0)
    <span class="text-dark fw-bold rounded-pill bg-danger text-white px-2 py-1" 
...
<div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list pb-0" 
  ...
  @if ($stocks->count() > 0)
    ...
    @foreach ($stocks as $item)
      @if (Auth::user()->role == 1)
        <a href="/admin/operator/stock/edit/{{ $item->id }}" class="dropdown-item preview-item py-3">
      @else
        <a href="/admin/stock/" class="dropdown-item preview-itssem py-3">
      @endif
        <div class="preview-thumbnail">
